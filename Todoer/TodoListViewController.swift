...
extension TodoListViewController: UISearchBarDelegate {
   ...
   func searchBarSearchButtonClicked(_ searchBar: UISearchBar) {
       ...
       let predicate = NSPredicate(format: "title Contains %@", searchBar.text!)      // title should contain whatever is in the search bar

       request.predicate = predicate

       let sortDescriptor = NSSortDescriptor(key: "title", ascending: true)           // results should come back with the titles in ascending order
       request.sortDescriptors = [sortDescriptor]

       do {
           itemArray = try context.fetch(request) //fetch results from persistant store and assign them to itemArray
       } catch {
           print("Error fetching data from context,\(error)")
       }
